...
class SkiTemplateComponent extends HTMLElement {
  ...
  get templateAttributes() {
    return Object.fromEntries(
      this.getAttribute('attributes')?.split(/,?\s+/).map(name => [name, undefined]) ?? [])
  }
  ...
  async createComponent(name?: string, extendsComponent?: string) {
    ...
    const modules = await Promise.all(
      Array.from(
        this.content.querySelectorAll<HTMLScriptElement>('script'))
          .map(module => (module.remove(), importScript(module, baseURI))))
