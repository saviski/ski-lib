...
export default class SkiDependencyEval {
  ...
  async *run() {
    do {
      ...
      try {
        ...
        isAsyncGenerator(result) ? yield* this.original.get(result) || result : yield this.original.get(result) || result
      } catch (e) {
        if (!(e instanceof Dependency)) throw Error(`Evaluating expression (${this.expression})\n${e}`)
      }
      await this.deps()
  ...
  private generators() {
    return Array.from(this.valueMap.keys()).filter(data => isAsyncGenerator(data) && SkiDependencyEval.generatorPromises.has(data))
  }
  ...
  get(target: any, property: PropertyKey, _receiver: any) {
    ...
    if (!(property in target)) throw ReferenceError(`${String(property)} is not defined in: ${target}`)

    let value = isAsyncGenerator<any>(target) ? this.map(target, property) : target[property]

    this.register(target, String(property), value)
    if (isAsyncGenerator(value)) value = this.generatorValue(value)
  ...
  generatorValue(generator: AsyncGenerator<any>) {
    if (SkiDependencyEval.generatorValues.has(generator)) return SkiDependencyEval.generatorValues.get(generator)
    else {
